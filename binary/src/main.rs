//! `main.rs` should contain top-level documentation for the whole crate.
//!
//! `main.rs` should contain documentation regarding the entire crate.  A common
...
#[allow(clippy::unnecessary_wraps,
        reason = "Clippy complains because no error is returned from main().  \
                  That is a spurious warning that should be ignored.")]
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("Replace the template contents with your own code.");
...
mod tests {
    ...
    #[test]
    fn testing_main() {
        main().expect("If this is failing, then the template is broken!");
    }
